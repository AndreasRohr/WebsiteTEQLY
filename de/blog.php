<?php
  ...
  $response = file_get_contents("${apiURL}/ghost/api/v3/content/posts/slug/${slug}/?key=${apiKey}&fields=title,html,feature_image");
  $post = json_decode($response)->posts[0];
?>
<head>
    <?php include '../de/structure_header_imports.php' ?>
...
<div style="margin-left:3.5rem; margin-right: 3.5rem" class="post">
...
var toggleButtons = document.querySelectorAll(".kg-toggle-card-icon");
...
<style>
  ...
  .kg-toggle-content.hidden {
    max-height: none;
  }

  .kg-toggle-card-icon.active {
    background: #ddd;
  }
</style>
